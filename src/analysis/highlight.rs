...
//! and `None` from it is what lets a constant or a call fall through.
//!
//! Constants and methods come from the graph through [`references`], scoped to one document.
//! Nothing new is computed for them; a constant is exact and a method is matched by name, which
//! is the same trade `textDocument/references` makes and is stated in the README in those words.
//! Confined to one file it is a much better trade than it is across a workspace: the other
//! `render` in this file really is likely to be the same `render`.
//!
...
use super::{locator, references, scopes};
...
#[must_use]
pub fn find(graph: &Graph, uri_id: UriId, source: &str, offset: u32) -> Vec<Highlight> {
    ...
    locator::locate(graph, uri_id, offset)
        ...
        .find_map(|located| {
            ...
            let found = references::find(graph, &located, &resolution, &scope, true);
