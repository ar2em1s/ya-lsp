...
- **`NOTICE.txt` reproduces `vendor/rbs/BSDL` and `COPYING` verbatim, and a test enforces it.**
  ...
  **`THIRD-PARTY-NOTICES.txt` covers the ~100 crates linked into the binary.** It is generated by
  ...
  so that `cargo build` and `cargo install` need no extra tool — regenerated and diffed byte for
  byte by `make notices-check`, which is what both workflows run rather than spelling out.
  **It is checked in two places because the two triggers do not overlap**: `ci.yml` runs on
  pushes to main/master and on pull requests, and a tag push fires neither, so `release.yml`
  has a `notices` job that `vsix` needs — otherwise a tag cut from a commit CI never saw ships
  twelve artifacts carrying a notice that omits a dependency, on the one path where nothing
  can be corrected afterwards. The version of `cargo-about` is pinned once, in the `Makefile`,
  and installed by `make setup-notices`. `about.toml`'s `accepted` list is ordered: the first entry matching a crate's
